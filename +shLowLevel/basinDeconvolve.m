...
%   [avgHat, out] = shLowLevel.basinDeconvolve(B, op) removes filter attenuation
...
for q = 1:numel(fn)
    val = chk.(fn{q});
    if ~all(isfinite(val(:)))
        error('shLowLevel:basinDeconvolve:nonFiniteOperator', ...
...
for t = 1:T
    ...
    if constrained
        ...
        A = B' * shLowLevel.opApply(op, B, t);
    else
        A = Gv' * (g .* Hu);                     % == B' W_t B
    end
    if ~all(isfinite(A(:)))
        error('shLowLevel:basinDeconvolve:nonFiniteA', ...
            'Non-finite A at month %d (s(t)=%.6g, g range [%.3g, %.3g]).', ...
            t, op.s(t), min(g), max(g));
    end
    if norm(A) == 0
        error('shLowLevel:basinDeconvolve:zeroA', ...
    ...
    if constrained
        ...
        WtB = shLowLevel.opApply(op, B, t, 'transp');
        Yv  = op.V' * WtB;
        Q = op.s(t) * (Yv' * Yv);
    else
