...
%
%   REP = shLowLevel.compareSolutions(G1, G2) compares two shCoefficients on a
...
%               (shLowLevel.spatialStats; Taylor-ready)
...
%                  .spectral (1,1) struct  shLowLevel.diffSpectrum output
%                  .spatial  (1,1) struct  shLowLevel.spatialStats output
...
% ---- spectral
spec = shLowLevel.diffSpectrum(g1.C, g1.S, g2.C, g2.S);
% ---- error realism (coefficient domain)
chi2dof = NaN;
if ~isempty(g1.sigmaC) && ~isempty(g2.sigmaC)
    idx = shLowLevel.shIndex(nmax);
    d = shLowLevel.vecFromCS(g1.C - g2.C, g1.S - g2.S, idx);
    s2 = shLowLevel.vecFromCS(g1.sigmaC, g1.sigmaS, idx).^2 + ...
         shLowLevel.vecFromCS(g2.sigmaC, g2.sigmaS, idx).^2;
...
B = g2.synthesis(opts.LatDeg, opts.LonDeg, quantity = opts.Quantity, ...
    kn = opts.kn);
st = shLowLevel.spatialStats(A, B, opts.LatDeg, opts.LonDeg, Mask = opts.Mask);
...
if opts.Plot
    ...
    nexttile;
    so = shLowLevel.shOrderRMS(g1.C - g2.C, g1.S - g2.S);
