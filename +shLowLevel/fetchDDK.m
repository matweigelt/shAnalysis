...
%
%   FILES = shLowLevel.fetchDDK(1:8) fetches the released anisotropic DDK
...
%
%   Load with W = shLowLevel.readDDK("DDK5") (resolves against Dest) or with
...
dest = opts.Dest;
if strlength(dest) == 0
    dest = string(fullfile(shLowLevel.dataFolder(), 'DDK'));
end
...
for k = unique(which(:)')
    ...
    try
        ...
        shLowLevel.readDDK(tmpf);                      % verify BEFORE swap
    catch err
        if isfile(tmpf), delete(tmpf); end
        if present
            % failed refresh: the existing file stays authoritative
            warning('shLowLevel:fetchDDK:updateFailed', 'DDK%d: %s', k, err.message);
            skipped(end+1) = string(fp); files(end+1) = string(fp); %#ok<AGROW>
            continue
        end
