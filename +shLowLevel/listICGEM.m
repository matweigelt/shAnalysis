...
%
%   T = shLowLevel.listICGEM() returns a table of the STATIC global models
...
%
%   T = shLowLevel.listICGEM(Type="temporal", Series=PATH) fetches the series
...
%   (For ITSG monthlies shLowLevel.fetchITSG remains the convenient route.)
%
%   Options
...
if listFiles && opts.Type ~= "temporal"
    error('shLowLevel:listICGEM:seriesNeedsTemporal', ...
...
if opts.Type == "static"
    ...
    assert(~isempty(name), 'shLowLevel:listICGEM:parseFailed', ...
        'No models parsed from %s - page layout may have changed.', src);
    T = table((1:numel(name))', name, year, degree, data, url, ...
        'VariableNames', {'idx', 'name', 'year', 'degree', 'data', 'url'});
elseif listFiles
    ...
    assert(~isempty(grp), 'shLowLevel:listICGEM:parseFailed', ...
        'No series parsed from %s - page layout may have changed.', src);
    T = table((1:numel(grp))', grp, ctr, ser, pth, url, zp, 'VariableNames', ...
        {'idx', 'group', 'center', 'series', 'path', 'url', 'zip'});
    info.note = ['Full temporal catalogue from the series-page links. ' ...
        ...
        'shLowLevel.fetchITSG remains the convenient route.'];
end
end
