...
%
%   T = shLowLevel.listITSG() enumerates the available ITSG releases and
...
%   row number is the selection handle for shLowLevel.fetchITSG(Catalog=...).
%   Behaviour mirrors shLowLevel.listICGEM.
%
%   Options
...
if isempty(rows)
    error('shLowLevel:listITSG:empty', 'No ITSG releases found under %s.', base);
end
K = size(rows, 1);
