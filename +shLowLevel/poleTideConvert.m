...
%
%   G2 = shLowLevel.poleTideConvert(G, From="IERS2010", To="IERS2018") adjusts
...
%   conversion explicit.
%
%   [C2, S2] = shLowLevel.poleTideConvert(C, S, EPOCH, ...) is the raw form on
...
if nargin >= 1 && isa(varargin{1}, 'shCoefficients')
    g = varargin{1};
    if ~isfinite(g.epoch)
        error('shLowLevel:poleTideConvert:noEpoch', ...
...
if size(C, 1) < 3 || ~isequal(size(C), size(S))
    error('shLowLevel:poleTideConvert:badSize', ...
