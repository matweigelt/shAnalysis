...
%
%   LN = shLowLevel.readLoveNumbers(FILE) parses a plain-text load-Love-number
...
if ~isfile(filename)
    error('shLowLevel:readLoveNumbers:fileNotFound', 'File not found: %s', ...
        filename);
end
if xor(strlength(opts.InFrame) > 0, strlength(opts.OutFrame) > 0)
    error('shLowLevel:readLoveNumbers:badFrame', ...
...
if isempty(vals)
    error('shLowLevel:readLoveNumbers:noData', ...
...
if any(n < 0 | n ~= round(n))
    error('shLowLevel:readLoveNumbers:badColumns', ...
...
if ~isempty(opts.MaxDegree)
    N = opts.MaxDegree;
    if n(1) > 0 || n(end) < N
        error('shLowLevel:readLoveNumbers:degreeGap', ...
...
if strlength(opts.OutFrame) > 0 && opts.InFrame ~= opts.OutFrame
    i1 = find(n == 1, 1);
    if isempty(i1)
        error('shLowLevel:readLoveNumbers:needsDegree1', ...
    ...
    switch pairKey
        ...
        otherwise   % "CF>CE", "CM>CE"
            error('shLowLevel:readLoveNumbers:frameNotRecoverable', ...
...
if isempty(toks)
    if ncol == 2
        ...
        error('shLowLevel:readLoveNumbers:ambiguousColumns', ...
