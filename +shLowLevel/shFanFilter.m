...
%
%   [CF, SF] = shLowLevel.shFanFilter(C, S, RDEGKM, RORDKM) applies the fan
...
nmax = size(C, 1) - 1;
Wn = shLowLevel.shGaussianWeights(nmax, rDegKm, R = opts.R / 1e3);  % expects km
Wm = shLowLevel.shGaussianWeights(nmax, rOrdKm, R = opts.R / 1e3);
