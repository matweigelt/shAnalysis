...
%
%   [Y, W, GRID] = shLowLevel.synthesisMatrix(IDX) returns Y (Ngrid x P) with
%   f = Y*x for coefficient vectors x in IDX ordering (shLowLevel.shIndex), and
...
%
%   Built on the single toolbox Legendre engine shLowLevel.legendreALF
...
[tGL, wGL] = shLowLevel.gaussLegendre(opts.NLat);
...
Pall = shLowLevel.legendreALF(idx.Lmax, lat);            % (L+1)x(L+1)xNLat
