...
for k = 1:T
    ...
    grids{k} = shLowLevel.shSynthesis(g.C, g.S, g.GM, g.R, args{:});
end
cl = opts.CLim;
if isempty(cl)
    allv = abs(cat(3, grids{:}));
    a = shLowLevel.pctile(allv, 98);
    if a == 0, a = 1; end
    cl = [-a, a];
end
...
switch lower(ext)
    case '.mp4'
        ...
        if ~any(strcmp({profs.Name}, 'MPEG-4'))
            error('shLowLevel:writeAnimation:mp4Unsupported', ...
    ...
    otherwise
        error('shLowLevel:writeAnimation:badExtension', ...
...
for k = 1:T
    cla(ax);
    shLowLevel.plotSHMap(grids{k}, opts.lat, opts.lon, ax = ax, CLim = cl, ...
