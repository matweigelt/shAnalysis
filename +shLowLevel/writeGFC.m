...
%
%   shLowLevel.writeGFC(FILENAME, C, S, GM, R) writes a static ICGEM-format
%   gravity field file readable by shLowLevel.shReadGFC (round-trip tested to
...
assert(isequal(size(C), size(S)) && size(C,1) == size(C,2), ...
    'shLowLevel:writeGFC:badInput', 'C and S must be square and equally sized.');
...
assert(fid > 0, 'shLowLevel:writeGFC:cannotOpen', 'Cannot open %s for writing.', ...
    char(filename));
