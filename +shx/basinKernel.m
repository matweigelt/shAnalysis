...
%                    Gaussian weights of this radius (shx.shGaussianWeights)
...
[mask, ~] = shx.evalMask(idx, region, BufferKm = opts.BufferKm, ...
...
b = Y' * (w .* mask);
if opts.TaperKm > 0
    Wn = shx.shGaussianWeights(idx.Lmax, opts.TaperKm);
    b = b .* Wn(idx.n + 1);
end
