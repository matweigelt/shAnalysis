...
%
%   [K, INFO] = shx.basinScaling(OP, B, TSMODEL) estimates the classic
...
%   shx.basinDeconvolve: deconvolution inverts the operator on a kernel
...
%                         (shx.shIndex); structs carry their own
...
if isstruct(op) && isfield(op, 'idx')
    ...
    applyOp = @(x, t) shx.opApply(op, x, t);
elseif isnumeric(op)
    assert(isfield(opts.idx, 'P') && ~isempty(opts.tYears), ...
        'shx:basinScaling:needIdx', ...
        'Matrix operators need idx= and tYears=.');
    idx = opts.idx; tY = opts.tYears;
    assert(isequal(size(op), [idx.P idx.P]), 'shx:basinScaling:badOp', ...
...
elseif isa(op, 'function_handle')
    ...
    error('shx:basinScaling:badOp', ...
        'Operator must be a tvANS struct, a P x P matrix, or a handle.');
end
assert(numel(b) == idx.P, 'shx:basinScaling:badKernel', ...
...
for t = 1:T
    ...
    x = shx.vecFromCS(Cm, Sm, idx);
    xf = applyOp(x, t);
    aTrue(t) = (b' * x) / bb;
    aFilt(t) = (b' * xf) / bb;
end
assert(nnz(matched) >= 3, 'shx:basinScaling:tooFewMatches', ...
