...
%   [N, info] = shx.buildNoiseCov(Xres, idx) estimates the noise
...
%       (symmetrized). Combine with shx.vceRescale for monthly scaling.
%
%   Options:
%     Mode       ('empirical') | 'full'
...
if strcmp(opts.Mode, 'full')
    ...
    assert(offMass <= 1e-24 * max(sum(Nf(:).^2), realmin), ...
        'shx:buildNoiseCov:notBlockDiagonal', ...
...
% returning Inf eigenvalues (diagnosed via shx:tvANSFilter:nonFiniteEig
...
dpos = dAll(dAll > 0);
if isempty(dpos)
    error('shx:buildNoiseCov:allZeroVariance', ...
