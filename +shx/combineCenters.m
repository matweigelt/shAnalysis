...
%
%   [TSCOMB, INFO] = shx.combineCenters({tsITSG, tsCSR, tsGFZ}) combines
...
%   own climatology residuals (shx.buildNoiseCov), TRACE-NORMALIZED to
...
C = numel(tsCell);
assert(C >= 2, 'shx:combineCenters:tooFewCenters', ...
    'Need at least 2 series (got %d).', C);
nmax = tsCell{1}.nmax;
for c = 2:C
    assert(tsCell{c}.nmax == nmax, 'shx:combineCenters:nmaxMismatch', ...
...
for c = 1:C
    ...
    for t = 1:ts.nEpochs
        X(:, t) = shx.vecFromCS(ts.Cs(:, :, t), ts.Ss(:, :, t), idx);
    end
    if ts.nEpochs > 8
        [~, Xres] = shx.fitDeterministicModel(X, ts.epochs);
    else
    ...
    Nc = shx.buildNoiseCov(Xres, idx, Assemble = 'blocks');
    if c == 1
...
assert(T >= 1, 'shx:combineCenters:noCommonEpochs', ...
...
for c = 1:C
    ...
    for t = 1:T
        if have(c, t)
            j = J(c, t);
            Xc{c}(:, t) = shx.vecFromCS(tsCell{c}.Cs(:, :, j), ...
                tsCell{c}.Ss(:, :, j), idx);
        end
    end
end
...
for t = 1:T
    [Cs(:, :, t), Ss(:, :, t)] = shx.csFromVec(xh(:, t), idx);
    [sCs(:, :, t), sSs(:, :, t)] = shx.csFromVec(sqrt(max(Hd(:, t), 0)), idx);
end
names = strings(1, C);
