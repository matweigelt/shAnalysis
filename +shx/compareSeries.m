...
%
%   REP = shx.compareSeries({TS1, TS2, ...}) compares N >= 2 shSeries
...
%     - correlation with AR(1)-corrected significance (shx.effectiveCorr)
...
%   [REP, H] = shx.compareSeries(..., Plot = true) adds a 4-panel
...
%     Basin ([]), Idx ([])  P x 1 basin vector + shx.shIndex struct:
...
N = numel(tsList);
if N < 2
    error('shx:compareSeries:needTwo', 'Need at least 2 series.');
end
for k = 1:N
    if ~isa(tsList{k}, 'shSeries')
        error('shx:compareSeries:badInput', ...
...
if nnz(common) < 12
    error('shx:compareSeries:tooFewEpochs', ...
...
if useBasin && isempty(opts.Idx)
    error('shx:compareSeries:basinNeedsIdx', ...
        'Basin= requires the matching Idx= (shx.shIndex struct).');
end
y = nan(T, N);
if useBasin
    b = opts.Basin(:);
    for k = 1:N
        ...
        for t = 1:T
            g = tk.at(t);
            y(t, k) = b' * shx.vecFromCS(g.C, g.S, opts.Idx);
        end
    end
else
...
for k = 1:N
    [~, ~, coef, ~, coefSigma] = shx.fitDeterministicModel( ...
...
for k = 2:N
    nse(k) = shx.nashSutcliffe(y(:, 1), y(:, k));
    ec = shx.effectiveCorr(y(:, 1), y(:, k));
...
if hasSig(1)
    for k = 2:N
        ...
        for t = 1:T
            ...
            d = shx.vecFromCS(gr.C - gk.C, gr.S - gk.S, idx);
            s2 = shx.vecFromCS(gr.sigmaC, gr.sigmaS, idx).^2 + ...
                 shx.vecFromCS(gk.sigmaC, gk.sigmaS, idx).^2;
...
if opts.Plot
    ...
    shx.taylorDiagram(std(y0), stds, cors, Labels = names(2:end), ...
