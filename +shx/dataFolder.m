...
%
%   F = shx.dataFolder()          returns the current data folder
%                                 (created on demand). Default:
%                                 <toolbox root>/data.
%   F = shx.dataFolder(PATH)      sets and persists a user folder
%                                 (MATLAB preference, survives sessions:
%                                 setpref('shAnalysis','dataFolder')).
%   F = shx.dataFolder("reset")   back to the default.
%
%   Consumers: shx.fetchITSG (itsg_series/), shx.fetchDDK (DDK/),
%   shx.fetchICGEM (icgem/), shx.readDDK("DDK<n>") name resolution, and
...
if ispref(group, prefName)
    ...
    root = fileparts(fileparts(mfilename('fullpath')));   % .../+shx -> root
