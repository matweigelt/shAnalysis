...
%
%   [MASK, GRID] = shx.evalMask(IDX, REGION) evaluates REGION on the
%   Gauss-Legendre quadrature grid of shx.synthesisMatrix(IDX) and
%   returns MASK (Ngrid x 1, in [0,1]) plus the GRID struct. Shared by
%   shx.basinKernel and shx.slepianBasis.
%
%   REGION forms:
...
%     grid  struct from shx.synthesisMatrix (latDeg, lonDeg per ring)
%
