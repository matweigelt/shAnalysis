...
%
%   FILES = shx.fetchTN() fetches the GRACE/GRACE-FO technical notes that
%   complete the standard processing chain into <dataFolder>/TN:
%     TN-14  GSFC SLR C20/C30 replacement       (shx.readTN14/applyTN14)
%     TN-13  geocenter (degree 1) per provider  (shx.readTN13/addDegree1)
...
%     Dest (fullfile(shx.dataFolder(), "TN"))
...
%
%   Example
%     shx.fetchTN(Update = true);            % monthly refresh, safe swap
%     tn13 = shx.readTN13(fullfile(shx.dataFolder(), "TN", ...
...
dest = opts.Dest;
if strlength(dest) == 0
    dest = string(fullfile(shx.dataFolder(), 'TN'));
end
...
for p = unique(opts.Providers)
    ...
    parsers{end+1} = @shx.readTN13; %#ok<AGROW>
end
if opts.TN14
    names(end+1) = "TN-14_C30_C20_SLR_GSFC.txt";
    parsers{end+1} = @shx.readTN14;
end
...
for k = 1:numel(names)
    ...
    try
        if mirror
            ...
            if ~isfile(src)
                error('shx:fetchTN:mirrorMissing', ...
...
if ~opts.Quiet && ~isempty(failed)
    warning('shx:fetchTN:failed', '%d file(s) failed:\n  %s', ...
