...
%
%   H = shx.plotSHMap(GRID, LAT, LON) renders a synthesized field the way
...
assert(isequal(size(grid), [numel(latDeg), numel(lonDeg)]), ...
    'shx:plotSHMap:badSize', 'GRID must be nlat x nlon matching LAT/LON.');

cl = opts.CLim;
if isempty(cl)
    a = shx.pctile(abs(grid), 98);
    if a == 0, a = 1; end
    cl = [-a, a];
end
