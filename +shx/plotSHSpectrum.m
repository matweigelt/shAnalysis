...
%     Domain is inferred from spec.domain: pass shx.shOrderRMS output
...
%               shx.shSpectralCrossover with a vertical line
%
%   Claude (Sonnet 4.6), 2026-07-11; merged into +shx: Claude (Fable 5),
...
for k = 2:numel(specs)
    ...
    assert(strcmp(dk, dom), 'shx:plotSHSpectrum:mixedDomains', ...
...
switch qty
    ...
    otherwise
        error('shx:plotSHSpectrum:badQuantity', 'Unknown Quantity: %s', qty);
end
if fieldExplicit
...
for k = 1:numel(plotSpecs)
    s = plotSpecs{k};
    assert(isfield(s, field), 'shx:plotSHSpectrum:missingField', ...
