...
%
%   W = shx.readDDK(SOURCE) loads a DDK filter into the toolbox's
%   order-block container, ready for shx.applyDDK / g.applyDDK /
...
%                          W = shx.readDDK('Wbd_...'); save ddk3.mat W)
...
if (ischar(source) || isstring(source)) && ...
    ...
    % the shipped tests/test_data (DDK3), else point at shx.fetchDDK
    sc = char(source);
    k = str2double(sc(4));
    fn = shx.ddkNames(); fn = fn(k);
    cand = [ ...
        string(fullfile(shx.dataFolder(), 'DDK', fn))
        string(fullfile(fileparts(fileparts(mfilename('fullpath'))), ...
            'tests', 'test_data', fn))];
    hit = cand(arrayfun(@isfile, cand));
    assert(~isempty(hit), 'shx:readDDK:notFetched', ...
        ['%s (%s) is not in %s or the shipped test data.\n' ...
         'Download it once with shx.fetchDDK(%d).'], ...
        upper(char(source)), fn, cand(1), k);
    W = shx.readDDK(hit(1), Nmax = opts.Nmax);
    return
end
if isstruct(source)
    W = source;
elseif (ischar(source) || isstring(source)) && endsWith(source, '.mat')
    assert(isfile(source), 'shx:readDDK:fileNotFound', 'File not found: %s', source);
    S = load(source);
    fn = fieldnames(S);
    assert(numel(fn) >= 1, 'shx:readDDK:emptyMat', 'MAT file holds no variables.');
    W = S.(fn{1});
...
elseif ischar(source) || isstring(source)
    ...
    while k <= numel(lines)
        ...
        if p(1) ~= "block" || numel(p) < 5
            error('shx:readDDK:badLine', 'Expected "block m cs n1 n2", got: %s', L);
        end
        ...
        for r = 1:nd
            ...
            v = v(~isnan(v));
            assert(numel(v) == nd, 'shx:readDDK:badMatrixRow', ...
    ...
    assert(~isempty(W.blocks), 'shx:readDDK:noData', 'No blocks found in %s.', source);
else
    error('shx:readDDK:badSource', 'SOURCE must be a struct, .mat file, or ASCII file.');
end
assert(isfield(W, 'blocks') && isfield(W, 'nmax'), 'shx:readDDK:badContainer', ...
...
fid = fopen(filename, 'r', 'ieee-le');
assert(fid > 0, 'shx:readDDK:fileNotFound', 'Cannot open %s', filename);
...
if marker ~= 18754
    ...
    marker = fread(fid, 1, 'uint16');
    assert(isequal(marker, 18754), 'shx:readDDK:badBinary', ...
...
assert(strncmp(mtype, 'BD', 2), 'shx:readDDK:badBinary', ...
...
assert(~isempty(Lmax), 'shx:readDDK:badBinary', 'No Lmax metadata found.');
if ndbls > 0
...
assert(numel(pack) == pval1, 'shx:readDDK:badBinary', ...
...
for iblk = 1:nblocks
    ...
    nmin = max(Lmin, m);
    assert(sz == Lmax - nmin + 1, 'shx:readDDK:badBinary', ...
...
assert(idx0 == pval1, 'shx:readDDK:badBinary', ...
