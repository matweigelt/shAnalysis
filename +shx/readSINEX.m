...
%
%   SNX = shx.readSINEX(FILENAME) parses the SINEX blocks used by
...
%
%   SNX = shx.readSINEX(..., Index=IDX) additionally reorders the result
%   into the shx.shIndex ordering IDX: SNX.M and SNX.x then match IDX row
...
%   from the file raise shx:readSINEX:missingParam.
%
%   Options
%     Index   ([]) shx.shIndex struct for reordering
...
if ~isfile(filename)
    error('shx:readSINEX:fileNotFound', 'File not found: %s', filename);
end
...
for k = 1:numel(lines)
    ...
    elseif startsWith(L, "+SOLUTION/MATRIX_ESTIMATE")
        ...
        if ~contains(L, "COVA")
            error('shx:readSINEX:badMatrixType', ...
    ...
    if inEst
        ...
        if isempty(it) || numel(tok) < it + 2
            error('shx:readSINEX:badEstimateLine', ...
    ...
    elseif inMat
        ...
        v = v(~isnan(v));
        if numel(v) < 3
            error('shx:readSINEX:badMatrixLine', ...
...
Q = numel(n);
if Q == 0
    error('shx:readSINEX:noData', 'No SOLUTION/ESTIMATE block found in %s.', filename);
end
...
if ~isempty(ijvC)
    ...
    if bad > Q
        error('shx:readSINEX:badMatrixIndex', ...
...
if ~isempty(fieldnames(opts.Index))
    ...
    for r = 1:idx.P
        ...
        if isempty(hit)
            error('shx:readSINEX:missingParam', ...
...
if useJava
    ...
    assert(fid > 0, 'shx:readSINEX:fileNotFound', 'Cannot open %s', filename);
...
assert(nEst > 0, 'shx:readSINEX:noEstimates', ...
