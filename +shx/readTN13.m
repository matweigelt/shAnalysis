...
%
%   TN = shx.readTN13(FILENAME) parses the official TN-13 product
...
if ~isfile(filename)
    error('shx:readTN13:fileNotFound', 'File not found: %s', filename);
end
...
for k = 1:numel(lines)
    ...
    if numel(p) < 9 || any(isnan(p(2:9)))
        error('shx:readTN13:badLine', 'Unparseable GRCOF2 line: %s', L);
    end
    rec(end+1) = struct('t0', p(8), 't1', p(9), 'n', p(2), 'm', p(3), ...
        'C', p(4), 'S', p(5), 'sC', p(6), 'sS', p(7)); %#ok<AGROW>
end
if isempty(rec)
    error('shx:readTN13:noData', 'No GRCOF2 records found in %s.', filename);
end
key = [[rec.t0]', [rec.t1]'];
[uk, ~, iu] = unique(key, 'rows', 'stable');
K = size(uk, 1);
tn.t0 = shx.icgemDate2Year(uk(:,1));
tn.t1 = shx.icgemDate2Year(uk(:,2));
