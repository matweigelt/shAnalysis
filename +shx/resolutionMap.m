...
%
%   RES = shx.resolutionMap(OP, T, LATDEG, LONDEG) evaluates, for each
...
%
%   Inputs
%     op       struct     from shx.tvANSFilter
%     t        (1,1)      month index
...
for p = 1:nP
    ...
    yP = shx.ylm(lat0, lon0, idx)';                   % P x 1
    k  = shx.opApply(op, yP, t, 'transp');
    k0 = k' * yP;
    ...
    V = reshape(shx.ylm(lat2(:), lon2(:), idx) * k, size(AZ));  % NAz x NPsi
