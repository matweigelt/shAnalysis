...
%
%   [S, GRID, INFO] = shx.seaLevelFingerprint(LOADREGION, OCEAN, IDX,
...
%     ocean       ocean region: handle | polygon | mask (shx.evalMask
%                 forms). The load is restricted to land (1 - ocean).
%     idx         shx.shIndex(Lmax, MinDegree = 0)  - MinDegree MUST be
...
assert(idx.minDegree == 0, 'shx:seaLevelFingerprint:badIndex', ...
...
[Y, w, grid] = shx.synthesisMatrix(idx, ...
    NLat = opts.OverSample * n1, NLon = opts.OverSample * (2 * idx.Lmax + 2));
[O, ~] = shx.evalMask(idx, ocean, OverSample = opts.OverSample);
...
elseif isnumeric(loadRegion) && numel(loadRegion) == numel(O)
    ...
    error('shx:seaLevelFingerprint:badLoad', ...
