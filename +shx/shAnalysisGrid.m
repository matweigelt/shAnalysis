...
%   from gridded or scattered data (the inverse of shx.shSynthesis).
%
...
%               quantity; the degree-dependent kernel (shx.kernelFactors)
...
%               rank deficiency raises shx:shAnalysisGrid:rankDeficient.
...
if opts.LatType == "geodetic"
    ...
    latVec = shx.geodetic2geocentric(latVec, Flattening = opts.Flattening);
end


lat = latVec(:); lon = lonVec(:);
kernel = shx.kernelFactors(opts.quantity, nmax, opts.GM, opts.R, ...
...
isScat = ~isRing && isvector(grid) && numel(grid) == numel(lat) ...
    && numel(lat) == numel(lon);
if ~isRing && ~isScat
    error('shx:shAnalysisGrid:badInput', ...
...
if method == "rings" && ~isRing
    error('shx:shAnalysisGrid:badInput', 'Method="rings" needs a ring grid.');
end
...
if method == "rings"
    ...
    if nlon < 2 || any(abs(dl - dl(1)) > 1e-9) || dl(1) <= 0 ...
        ...
        error('shx:shAnalysisGrid:badGrid', ...
    ...
    P = shx.legendreALF(nmax, latRad);         % (n+1)x(m+1)xnlat
    w = ones(nlat, 1);
    ...
    idx = shx.shIndex(nmax, MinDegree = 0);
    ...
    for i0 = 1:opts.ChunkSize:npts
        ...
        Y = shx.ylm(latRad(ii), lonRad(ii), idx);       % npts_chunk x P
        Y = Y .* kernel(idx.n + 1)';
        Y = Y(:, est);
        ATA = ATA + Y' * (w2(ii) .* Y);
    ...
    [R_, flag] = chol(ATA);
    if flag ~= 0
        error('shx:shAnalysisGrid:rankDeficient', ...
    ...
    [C, S] = shx.csFromVec(xf, idx);
    % residual RMS (second pass, chunked)
    residSS = 0;
    for i0 = 1:opts.ChunkSize:npts
        ii = i0:min(i0 + opts.ChunkSize - 1, npts);
        Y = shx.ylm(latRad(ii), lonRad(ii), idx) .* kernel(idx.n + 1)';
