...
%
%   STATS = shx.spatialStats(A, B, LATDEG, LONDEG) compares two
...
%     st = shx.spatialStats(A1, A2, lat, lon);
...
if ~isequal(size(A), size(B), [numel(latDeg), numel(lonDeg)])
    error('shx:spatialStats:sizeMismatch', ...
...
if isempty(opts.Weights)
    ...
    if ~isequal(size(w), size(A))
        error('shx:spatialStats:badWeights', ...
...
w(~use) = 0;
if sum(w(:)) <= 0
    error('shx:spatialStats:emptyMask', 'No usable grid points.');
end
w = w / sum(w(:));
