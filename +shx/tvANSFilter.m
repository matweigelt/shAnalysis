...
%
%   [XF, OP, INFO] = shx.tvANSFilter(X, TYEARS, IDX) runs the tvANS chain
%   on the coefficient series X (P x T, IDX ordering, shx.shIndex):
%
...
%                           (use with shx.opApply / shx.basinDeconvolve /
%                           shx.resolutionMap)
...
assert(isequal(size(X), [P T]), 'shx:tvANSFilter:badSize', ...
    'X must be P x T in idx ordering.');

% 1. deterministic separation
[model, Xres, ~, Afit, ~, detResVar] = shx.fitDeterministicModel( ...
...
switch opts.Blocks
    case 'on'
        assert(canBlock, 'shx:tvANSFilter:blocksUnavailable', ...
...
if isempty(opts.NoiseCov)
    if useBlocks
        [N, infoN] = shx.buildNoiseCov(Xres, idx, ...
            Shrinkage = opts.Shrinkage, Assemble = 'blocks');
    else
        [N, infoN] = shx.buildNoiseCov(Xres, idx, Shrinkage = opts.Shrinkage);
    end
else
    if useBlocks
        try
            [N, infoN] = shx.buildNoiseCov(Xres, idx, Mode = 'full', ...
                FullCov = opts.NoiseCov, Assemble = 'blocks');
        catch ME
            if strcmp(ME.identifier, 'shx:buildNoiseCov:notBlockDiagonal') ...
...
s = shx.vceRescale(N, Xres, idx, MinDegree = opts.VCEMinDegree);
assert(all(isfinite(s)), 'shx:tvANSFilter:nonFiniteVCE', ...
    'Non-finite VCE factors at months: %s', mat2str(find(~isfinite(s))'));
banded = ~isempty(opts.VCEBands);
if banded
    assert(useBlocks, 'shx:tvANSFilter:bandsNeedBlocks', ...
    ...
    for q = 1:nBand
        ...
        if any(inB & (idx.n >= opts.VCEMinDegree))
            sBand(q, :) = shx.vceRescale(N, Xres, idx, ...
                MinDegree = opts.VCEMinDegree, Rows = inB);
        else
            sBand(q, :) = s;                    % band empty above MinDegree
        end
    end
    assert(all(isfinite(sBand(:))), 'shx:tvANSFilter:nonFiniteVCE', ...
        'Non-finite band VCE factors.');
end

% 4. signal covariance
[S, infoS] = shx.buildSignalCov(Xres, N, idx, ...
...
if useBlocks
    ...
    for k = 1:nb
        ...
        if ~all(isfinite(blocks(k).lam)) || ~all(isfinite(Ub(:))) ...
            ...
            error('shx:tvANSFilter:nonFiniteEig', ...
...
Xfres = zeros(P, T);
for t = 1:T
    Xfres(:, t) = shx.opApply(op, Xres(:, t), t);
end
op.Xfres = Xfres;
Xf = Xfres + model;
