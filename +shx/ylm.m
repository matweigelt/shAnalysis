...
%
%   Y = shx.ylm(LATRAD, LONRAD, IDX) evaluates the basis at nPts points
...
%   Vectorized over points via one shx.legendreALF call.
%
%   Inputs
...
assert(numel(latRad) == numel(lonRad), 'shx:ylm:sizeMismatch', ...
    'latRad and lonRad must have equal length.');

nPts = numel(latRad);
Pall = shx.legendreALF(idx.Lmax, latRad);         % (L+1)x(L+1)xnPts
