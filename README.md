...
**`shLowLevel.readLoveNumbers` (new).** Load-Love-number reader: 2-column and
...
**Comparison suite (v2.6.0).** `shLowLevel.compareSolutions(g1, g2)` and
`shLowLevel.compareSeries({ts1, ts2, ...})` (or `g.compare(g2)` / `ts.compare(...)`)
...
(`shLowLevel.taylorDiagram` also stands alone). Each metric is its own function for
...
TN-13 GFZ/CSR/JPL via the new `shLowLevel.fetchTN`, every file verified by
...
**ICGEM temporal catalogue (bug fix + extension).** `shLowLevel.listICGEM(Type=
...
cumvariance) and order-domain support via **`shLowLevel.shOrderRMS`** (the striping axis; degree/order
marginals verified to sum to the same total). `shLowLevel.shDegreeRMS` carries `cumVariance`/`cumRMS` now.

Data management: **`shLowLevel.dataFolder`** (persistent user folder; all fetchers store beneath it),
**`shLowLevel.fetchDDK`** + `shLowLevel.readDDK("DDK<n>")` for **all eight** released DDK filters
(DDK3 still ships in the package), **`shLowLevel.listICGEM`** (180+ static models as a table;
fixture-tested parser) and **`shLowLevel.fetchICGEM("<name>")`** for direct .gfc downloads.
`shLowLevel.fetchITSG` now targets `dataFolder/itsg_series` by default and gained
...
**Visualization** — `shLowLevel.plotSHMap`/`g.map` (divergent scale, coastlines, analytic Hammer
projection, no Mapping Toolbox), `shLowLevel.plotBasinSeries` (1σ band, mission-gap patches,
AR(1)-trend annotation), signed **triangle differences** (`'RefC'/'RefS'`),
`shLowLevel.plotCovariance` (block-boundary overlay), `plotSHSpectrum` Kaula + crossover
overlays, `shLowLevel.writeAnimation` (MP4, fixed robust color scale).

**Computation** —
- `shLowLevel.errorMap`: analytic σ-maps from full covariances (Cholesky; matches `mcPropagate`).
- `shLowLevel.eofAnalysis`: EOF modes as `shCoefficients`, unit-variance PCs, variance fractions.
- `shLowLevel.basinScaling`: forward-modelled gain factors; operators as tvANS struct, matrix, or handle.
...
- `shLowLevel.shFanFilter` / `g.fan` / `ts.fan`: Han degree×order Gaussian.
- `shLowLevel.combineCenters`: **multi-center VCE** with one factor per (center, month) and
...
- `shLowLevel.shSynthesisGradientTensor`: all six NEU components at altitude; Laplace trace
  ≤ 7e-16 built in as self-check; exact second Legendre derivatives (stencil²).
- `shLowLevel.seaLevelFingerprint`: elastic sea-level equation on the exact quadrature pair;
...
- `shLowLevel.writeGrid`: CF-style netCDF (2-D/3-D), roundtrip-tested.
- **Reference systems**: `shLowLevel.normalFieldCS` computes the even zonals from the WGS84/GRS80
  **defining constants** (Heiskanen-Moritz closed form; matches NIMA TR8350.2 to all published
  ...
  `shLowLevel.rescaleGMR`/`g.toReference` convert between conventions exactly (C′=C·(GM₁/GM₂)(R₁/R₂)ⁿ;
...
- **Real data on demand**: `shLowLevel.fetchITSG(2010:2016)` downloads ITSG monthly solutions
...
- `shLowLevel.pctile`: percentile in base MATLAB (audit caught `prctile`/Statistics Toolbox in two
...
- **`shLowLevel.shSynthesisDeformation` / `g.deformation`** — up/north/east elastic load
  deformation (Wahr et al. 1998), grid or station-list (`Mode="points"`) evaluation,
  `LatType="geodetic"` for GNSS coordinates. Exact ∂P̄/∂φ via `shLowLevel.legendreALFDeriv`
...
- **DDK filters** — `shLowLevel.readDDK` reads the released binary `Wbd_*` files natively
...
- **Basin kernels** — `shLowLevel.basinKernel(idx, polygon|mask|fun, BufferKm=, TaperKm=)`
  replaces manual `Y'*(w.*mask)`; exact great-circle buffering, Jekeli spectral taper.
- **Slepian localization** — `shLowLevel.slepianBasis`: concentration eigenproblem, Shannon
...
- **Geodetic latitude** — `shLowLevel.geodetic2geocentric` / inverse, plus
...
- **Monte-Carlo propagation** — `shLowLevel.mcPropagate(fun, g, Cov=, Idx=)`: empirical
...
- **Mascon reader** — `shLowLevel.readMascon` (base-MATLAB `ncread`), JPL/CSR layout
...
P = shLowLevel.legendreALF(2190, deg2rad(80));               % scaled recursion, stable

% released covariances / normal equations (ITSG/COST-G SINEX)
snx = shLowLevel.readSINEX("solution.snx", Output="covariance", Index=shLowLevel.shIndex(60));
...
* `shLowLevel.readTN14` assumes the standard 10-column TN-14 layout; TN-13
