...
ser = 'E:\DATAPOOL\GravityField\series\02_COST-G__COST-G_Grace-Grace-FO_RL02.1';
...
fprintf(['%s  oceanChain+EOF %.0f s: %d modes, circ RMS %.4f m, ' ...
    ...
    ternary(out.nModes >= 1 && out.sigMon < 0.0149 ...
        && abs(out.trend - 1.557) < 0.05, "PASS", "FAIL"), ...   % buffer-300 default baseline (v3.16.1)
...
%% 6e - v3.15/16 coastal-leakage controls (live-baselined 2026-08-13)
% Reference numbers from the machine run: base +1.407 (the published
% acceptance), buffer 300 km +1.557 (area 85.4%), buffer 500 km
% +1.647 (area 73.0%) - the trend walks INTO the published barystatic
% band ~1.6-2.2 exactly as the leakage model predicts. Removal(iter1)
% + 300 km: +1.524 - slightly BELOW buffer-only, correct physics:
% "outside" includes the polar caps, whose real Arctic-positive
% signal no longer leaks in. Full iter5 removal is a batch job
% (~10 field ops/epoch); iter1 carries most of the effect
% (pre-validated 0.045 vs 0.061).
try
    base = shLowLevel.oceanChain(ser, kn = kn, OceanMask = oc, ...
        GADFolder = 'E:\DATAPOOL\GravityField\GAX\GAD', ...
        GAAFolder = 'E:\DATAPOOL\GravityField\GAX\GAA', ...
        CoastBufferKm = 0, Quiet = true);   % pre-v3.16.1 reference
    fprintf('%s  6e buffer 0 (legacy ref): %+.3f mm/yr (ref +1.407 +- 0.05)\n', ...
        ternary(abs(base.trend - 1.407) < 0.05, "PASS", "CHECK"), base.trend);
    for buf = [300, 500]
        o = shLowLevel.oceanChain(ser, kn = kn, OceanMask = oc, ...
            GADFolder = 'E:\DATAPOOL\GravityField\GAX\GAD', ...
            GAAFolder = 'E:\DATAPOOL\GravityField\GAX\GAA', ...
            CoastBufferKm = buf, Quiet = true);
        ref = 1.557 * (buf == 300) + 1.647 * (buf == 500);
        fprintf('%s  6e buffer %d km: %+.3f mm/yr (ref %+.3f +- 0.05, area %.1f%%)\n', ...
            ternary(abs(o.trend - ref) < 0.05, "PASS", "CHECK"), ...
            buf, o.trend, ref, 100 * o.oceanArea / base.oceanArea);
    end
    o2 = shLowLevel.oceanChain(ser, kn = kn, OceanMask = oc, ...
        GADFolder = 'E:\DATAPOOL\GravityField\GAX\GAD', ...
        GAAFolder = 'E:\DATAPOOL\GravityField\GAX\GAA', ...
        RemoveLandLeakage = true, LandLeakIter = 1, Quiet = true);
    fprintf('%s  6e removal(1)+default300: %+.3f mm/yr (ref +1.524 +- 0.05)\n', ...
        ternary(abs(o2.trend - 1.524) < 0.05, "PASS", "CHECK"), o2.trend);
    % iter5 ladder point measured 2026-08-13 (368 s, detached batch):
    % +1.321 mm/yr - support ambiguity attributes coastal ocean signal
    % to land at depth; default stays iter1, deeper sweeps documented
    % as aggressive.
catch ME
